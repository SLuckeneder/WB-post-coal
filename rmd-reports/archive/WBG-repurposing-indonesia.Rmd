...
thresholds <- c(160, 10000, 10000, 2500, 4, 1)
...
We identified candidate areas for potential repurposing - particularly for applying the LURA tool - based on four criteria:

1. **Sufficient size** – Relatively large areas to allow flexible and impactful repurposing.
2. **Proximity to existing economic activity** – Within 5 km of a residential area (please keep in mind that OpenStreetMap (OSM) data are incomplete but provide a useful proxy - see histograms and maps above).
3. **Distance from sensitive sites** – At least 1 km from a Protected Area, Key Biodiversity Area (KBA), World Heritage Site (WHS) or Ramsar site.
4. **Repurposing potential** – Suitable for one or more repurposing options identified in this study.

```{r, echo=FALSE, warning = FALSE, results='asis'}
suitable <- cm_polygons_extended %>%
  dplyr::filter(country_name == cname) %>%
  dplyr::filter(dist_residential_m <= 5000, dist_KBA_PA_m > 1000) %>%
  dplyr::select(id, area_mine_ha, avg_GHI, avg_wind_speed_ms_50m) %>%
