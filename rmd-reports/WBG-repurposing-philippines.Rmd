...
variables_to_plot <- c("area_mine_ha", 
                       ...
                       "dist_urban_m", "avg_elevation",
                       "dist_KBA_m", "dist_PA_m", "dist_KBA_PA_m",
...
conc <- data.frame(
  ...
  varname_long = c("Mining area (ha)",
                   ...
                   "Dist. urban area (m)", "Avg. elevation (m)", 
                   "Dist. KBA (m)", "Dist. PA (m)", "Dist. KBA or PA (m)",
...
tdat <- cm_polygons_extended %>%
  ...
  st_drop_geometry() %>%
  select(all_of(variables_to_plot[variables_to_plot != "dist_KBA_PA_m"])) 
...
for (j in variables_to_plot[variables_to_plot != "dist_KBA_PA_m"]){
  p <- pdat %>%
    dplyr::filter(country_name == cname,
                  variable == j) %>%
    ggplot(aes(x = value)) +
...
for (j in seq_along(exclusion_factors)) {
  ...
  if(exclusion_factors[j] == "dist_KBA_PA_m") {breaks <- c(0, 1, breaks[-1])}
  ...
  if (exclusion_factors[j] %in% c("area_mine_ha", "dist_airport_m", "avg_GHI", "dist_KBA_PA_m")) {
    df <- df %>%
  ...
  # Plot
      ...
      if (exclusion_factors[j] %in% c("dist_KBA_PA_m")){
...
cowplot::plot_grid(plotlist = store, ncol = 3)
...
### Dataset of suitable mine sites for solar repurposing

```{r, echo=FALSE, warning = FALSE, results='asis'}

cm_polygons_suitable <- cm_polygons_flags %>%
  filter(id %in% suitable_solar$id)

cm_polygons_suitable %>%
  dplyr::select(1:7) %>%
...
# sum(cm_polygons_suitable_small$area_mine_ha)
...
These areas sum up to `r round(sum(cm_polygons_suitable_small$area_mine_ha), 0)` hectares.
...
for (j in seq_along(exclusion_factors)) {
  ...
  # Add fill colour based on thresholds
  if (exclusion_factors[j] %in% c("area_mine_ha", "dist_airport_m", "avg_wind_speed_ms_50m", "dist_KBA_PA_m")) {
    df <- df %>%
      mutate(fill_col = ifelse(bin_mid <= thresholds[j], "#e91e63", "#00bcd4"))
  } else {
    df <- df %>%
...
### Dataset of suitable mine sites for wind repurposing

```{r, echo=FALSE, warning = FALSE, results='asis'}

cm_polygons_suitable <- cm_polygons_flags %>%
  filter(id %in% suitable_wind$id)
...
knitr::kable(
  data.frame(
    Variable = c(
      "Pumped hydro dams locations from Weber et al. 2024",
      "Protected Areas, Key Biodiversity Areas, World Heritage Site, Ramsar Sites"
      ),
    Criterion = c(
      "areas without potential dam location excluded",
      "within excluded"
      )
  ),
  col.names = c("Variable", "Criterion"),
...
for (j in seq_along(exclusion_factors)) {
  ...
  # Plot
      ...
      if (exclusion_factors[j] %in% c("dist_PHES_upper_m", "dist_PHES_lower_m", "dist_KBA_PA_m")){
        p <- ggplot(df, aes(x = bin_max, y = count, fill = fill_col)) +
          geom_bar(stat = "identity") +
          scale_fill_identity() +
          theme_minimal() +
...
### Dataset of suitable mine sites for pumped hydro repurposing

```{r, echo=FALSE, warning = FALSE, results='asis'}

cm_polygons_suitable <- cm_polygons_flags %>%
  filter(id %in% suitable_phes$id)
...
## Additional repurposing suggestion: Restoration

**Mines within a Protected Area, KBA, WHS or Ramsar site:**
```{r, echo=FALSE, warning = FALSE, results='asis'}
cm_polygons_extended %>%
  dplyr::filter(country_name == cname) %>%
  st_drop_geometry() %>%
  dplyr::filter(dist_KBA_PA_m == 0) %>%
  dplyr::select(id, dist_KBA_PA_m, area_mine_ha) %>%
    dplyr::mutate(
    dist_KBA_PA_m = round(dist_KBA_PA_m, 0),
    area_mine_ha  = round(area_mine_ha, 0)
  ) %>%
...
**Mines outside but within 5 km of a Protected Area, KBA, WHS or Ramsar site:**
```{r, echo=FALSE, warning = FALSE, results='asis'}
cm_polygons_extended %>%
  dplyr::filter(country_name == cname) %>%
  st_drop_geometry() %>%
  dplyr::filter(dist_KBA_PA_m > 0 & dist_KBA_PA_m <= 5000) %>%
  dplyr::select(id, dist_KBA_PA_m, area_mine_ha) %>%
    dplyr::mutate(
    dist_KBA_PA_m = round(dist_KBA_PA_m, 0),
    area_mine_ha  = round(area_mine_ha, 0)
...
<!-- No mines were found to be suitable for commercial repurposing. However, three mines were identified as potentially interesting for further exploration regarding small-scale community approaches (see above). -->
...
mapview(
  ...
  col.regions = c("#00FFFF"), # only PHES
